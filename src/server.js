...
].scripts.map(script => `/${config.dir.js}/${script.file.split('/').pop()}`);

const styles = config[
  __DEVELOPMENT__ ? 'development' : 'production'
].styles.map(style => `/${config.dir.css}/${style.split('/').pop()}`);
...
const render = (req, res) => {
  ...
  configureStore({ isServer: true, location })
    .then(store => {
      res.send(
        `<!DOCTYPE html>${renderToStaticMarkup(<Page app={renderToString(<Root store={store} />)} meta={DocumentMeta.renderAsReact()} scripts={scripts} styles={styles} />)}`
      );
    })
